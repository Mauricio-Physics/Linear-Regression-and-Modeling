...
evals %>%
summarise(Median = median(score), Below_three = sum(score < 3), four_six = sum(score > 4.6) / n() )
```
...
ggplot(data = lm(score ~ bty_avg, data = evals), aes(x = .fitted, y = .resid)) +
  geom_point() +
...
```
```{r hist-res}
ggplot(data = lm(score ~ bty_avg, data = evals), aes(x = .resid)) +
  geom_histogram(binwidth = 0.2) +
  xlab("Residuals")
```
